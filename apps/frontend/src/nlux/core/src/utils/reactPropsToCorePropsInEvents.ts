import type { AiChatPropsInEvents, PersonaOptions } from '@nlux/core'
import type { AiChatProps } from '../exports/props'
...
export const reactPropsToCorePropsInEvents = <AiMsg>(
  ...
  if (props.personaOptions) {
    ...
    if (props.personaOptions.assistant) {
      result.personaOptions.assistant = {
        ...
        avatar:
          typeof props.personaOptions.assistant.avatar === 'string'
    ...
    if (props.personaOptions.user) {
      result.personaOptions.user = {
        ...
        avatar:
          typeof props.personaOptions.user.avatar === 'string'
  ...
  if (props.conversationOptions?.conversationStarters) {
    result.conversationOptions = {
      ...
      conversationStarters: props.conversationOptions.conversationStarters.map(
        starter => ({
          ...starter,
