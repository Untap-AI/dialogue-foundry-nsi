...
const MarkdownSnapshotRendererImpl = (props: MarkdownSnapshotRendererProps) => {
  ...
  useEffect(() => {
    if (
      ...
      markdownOptions?.showCodeBlockCopyButton !== false
    ) {
  ...
  const trustedHtml = useMemo(() => {
    return markdownOptions?.htmlSanitizer
      ? markdownOptions.htmlSanitizer(parsedContent)
      : parsedContent
  ...
  return (
    <MarkdownParserErrorBoundary>
      <div
        className={`nlux-markdownStream-root${editableStyle ? ` ${editableStyle}` : ''}`}
      >
        <div
...
export const MarkdownSnapshotRenderer = (
  props: MarkdownSnapshotRendererProps
) => {
  return (
    <MarkdownParserErrorBoundary>
