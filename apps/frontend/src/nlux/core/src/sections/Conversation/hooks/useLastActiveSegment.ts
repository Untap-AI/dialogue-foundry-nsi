...
export const useLastActiveSegment = function <AiMsg>(
  ...
  onLastActiveSegmentChange?: (
    data: { uid: string; div: HTMLDivElement } | undefined
  ) => void
) {
  ...
  const lastCallbackData = useRef<
    { uid: string; div: HTMLDivElement } | undefined
  >(undefined)
  ...
  useEffect(() => {
    ...
    if (
      lastActiveSegmentId === lastReportedData?.uid &&
    ...
    const data =
      lastActiveSegmentId && lastSegmentContainerRef.current
        ? {
