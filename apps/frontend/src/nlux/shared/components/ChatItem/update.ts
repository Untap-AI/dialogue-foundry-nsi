...
import type { ChatItemProps } from './props'
import type { AvatarProps } from '../Avatar/props'
import type { DomUpdater } from '../../types/dom/DomUpdater'
...
export const updateChatItemDom: DomUpdater<ChatItemProps> = (
  ...
  if (propsBefore.layout !== propsAfter.layout) {
    applyNewLayoutClassName(element)
  }
  ...
  if (
    ...
    const messageDom = element.querySelector<HTMLElement>(
      `.${messageClassName}`
    )
    if (messageDom) {
      updateMessageDom(
  ...
  if (propsBefore.name !== propsAfter.name) {
    const participantNameContainer = element.querySelector<HTMLElement>(
      `.${participantNameClassName}`
    )
