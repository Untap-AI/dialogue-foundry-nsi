...
export const createConversationScrollHandler = (
  callback: ConversationScrollCallback
): EventListener => {
  ...
  return (event: Event) => {
    ...
    const scrolledToBottom =
      Math.ceil(scrollTop + clientHeight) >= minExpectedScrollTop

    const scrollDirection =
      lastScrollTop === undefined || lastHeight === undefined
        ? undefined
        : scrollTop > lastScrollTop && lastHeight === scrollHeight
          ? 'down'
          : scrollTop < lastScrollTop && lastHeight === scrollHeight
            ? 'up'
            : undefined
