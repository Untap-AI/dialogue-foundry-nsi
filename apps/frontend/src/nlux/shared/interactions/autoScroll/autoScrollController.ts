...
import type { AutoScrollController } from './type'
...
export const createAutoScrollController = (
  ...
  let activeChatSegment: { uid: string; container: HTMLElement } | undefined =
    undefined

  const scrollHandler = throttle(
    createConversationScrollHandler(({ scrolledToBottom, scrollDirection }) => {
  ...
  const resetConversationContainer = (
    oldConversationContainer?: HTMLElement
  ) => {
  ...
  const handleActiveChatSegmentResized = () => {
    if (
      ...
      shouldScrollWhenGenerating &&
      scrollingStickToConversationEnd
    ) {
      scrollToBottom()
    }
  }
