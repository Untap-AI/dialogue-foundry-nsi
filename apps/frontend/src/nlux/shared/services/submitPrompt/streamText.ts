import { isStandardChatAdapter } from '../../types/adapters/chat/standardChatAdapter'
...
import type { StandardChatAdapter } from '../../types/adapters/chat/standardChatAdapter'
import type { ChatSegment } from '../../types/chatSegment/chatSegment'
import type { AiStreamedMessage } from '../../types/chatSegment/chatSegmentAiMessage'
import type {
    ...
    const observer: StreamingAdapterObserver<unknown> = {
      next: (chunk: unknown) => {
        ...
        if (isStandardAdapter) {
          ...
          const adapterAsStandardAdapter =
            adapter as unknown as StandardChatAdapter<AiMsg>
          const preProcessedChunk =
            adapterAsStandardAdapter.preProcessAiStreamedChunk(
              chunkAsRaw,
              extras
            )
